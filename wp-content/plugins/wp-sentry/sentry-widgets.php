...
if (class_exists('WP_Widget')) {
	class Sentry_Widget_Pages extends WP_Widget {
		...
		function widget($args, $instance) {
	            ...
	            $visible_pages_q = "SELECT * FROM {$wpdb->posts} WHERE post_type='page' AND !FIND_IN_SET(ID, '$exclude') AND (" . $WP_SENTRY->query_mod("{$wpdb->posts}.post_status = 'publish'") . ") ORDER BY $sortby";
	 ...
	 * @since 0.8
	 */        
	class Sentry_Widget_Recent_Comments extends WP_Widget {
	
		function Sentry_Widget_Recent_Comments() {
	...
	<?php
		...
		function widget( $args, $instance ) {
	            global $wpdb, $comments, $comment, $post, $WP_SENTRY;
	            ...
	            if (current_user_can('edit_others_posts')) {
	                ...
	                $visible_posts_q = "SELECT ID FROM {$wpdb->posts} WHERE " . $WP_SENTRY->query_mod("{$wpdb->posts}.post_status = 'publish'");
